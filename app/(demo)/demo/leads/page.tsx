...
function DemoLeadsPageContent() {
  ...
  const { stats: derivedStats } = useMemo(() => deriveAnalytics(leads), [leads])
  ...
  const stats = useMemo(() => {
    return [
      ...
      },
      {
        ...
        value: `$${derivedStats.pipelineValue.toLocaleString()}`,
        icon: BarChart3,
      ...
      {
        title: "Active Leads",
        value: (derivedStats.totalLeads - derivedStats.wonLeads).toLocaleString(),
        icon: Clock,
    ...
    <div className="p-6 md:p-12 space-y-10 md:space-y-16 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        ...
        <div className="flex items-center gap-4 w-full md:w-auto">
          {!isInitialized ? <Skeleton className="h-11 w-32 rounded-xl bg-secondary/20" /> : <DemoLeadsTableWrapper />}
        </div>
      </div>
          ...
          [1, 2, 3, 4].map(i => (
            <Card key={i} className="border-border/50 bg-card/30 backdrop-blur-sm shadow-none rounded-2xl overflow-hidden p-6 space-y-6">
              <div className="flex items-center justify-between">
                <Skeleton className="h-4 w-20 bg-secondary/20" />
                <Skeleton className="h-8 w-8 rounded-lg bg-secondary/20" />
              </div>
              <div className="space-y-3">
                <Skeleton className="h-10 w-24 bg-secondary/20" />
                <Skeleton className="h-3 w-28 bg-secondary/20 opacity-50" />
              </div>
            </Card>
          ))
        ) : (
      ...
      <div className="space-y-10">
        ...
        {!isInitialized ? (
          <div className="p-8 border border-border/50 rounded-2xl bg-card/20 space-y-6">
            <div className="flex items-center justify-between border-b border-border/30 pb-4">
              {[1, 2, 3, 4, 5].map(i => (
                <Skeleton key={i} className="h-4 w-28 bg-secondary/10" />
              ))}
            </div>
            {[1, 2, 3, 4, 5, 6, 7, 8].map(i => (
              <div key={i} className="flex items-center justify-between gap-4">
                <Skeleton className="h-10 w-full bg-secondary/5 rounded-lg" />
              </div>
            ))}
          </div>
        ) : (
